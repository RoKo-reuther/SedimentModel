...
pseudoOptim_mod <- function (f, p, ..., lower, upper, control = list() ) {
  ...
  iter <- 0
  lastbest  <- -Inf
  while (iter < numiter && (max(populationcost, na.rm = TRUE) - min(populationcost, na.rm = TRUE))
         > (min(populationcost, na.rm = TRUE)*varleft)) {
    iter <- iter + 1
    ...
    if(!is.nan(newcost) & !is.na(newcost) & !is.null(newcost)) {
      if (newcost < worstcost) {
        ...
        worstcost  <- populationcost[iworst]
        bestcost   <- min(populationcost, na.rm = TRUE)
        if (bestcost != lastbest)
          rsstrace  <- rbind(rsstrace, c(iter, min(populationcost, na.rm = TRUE)))
        lastbest <- bestcost
      }
    }
  } # end while loop
