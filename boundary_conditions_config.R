...
boundary_conditions <- list(
  ...
  constant = list(
    ...
    F_OrgCA       = 2.5,          # Org; Highly reactive: Check Multy G model approach (Westrich and Berner, 1984); 2.5-13
    F_OrgCB       = 1,          # Org; less reactive
    F_OrgCC       = 2,          # Org; non reactive
  ...
  varying = list(
    ...
    F_FeOH3A = Vectorize(function(v){
      ...
      if (round(v%%1, digits=15) >= round(1/12, digits=15) && round(v%%1, digits=15) < round(2/12, digits=15)) {
        flux <- flux + 21.6
