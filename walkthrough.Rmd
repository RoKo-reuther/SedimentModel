...
ss_combined_profiles <- reactive(handlers$combined_profiles_ss(ss_data=reac_vals$ss))
  ...
  tabsetPanel(
    ...
    tabPanel(title="reaction rate profiles",
      div(
        ...
        renderPlot(ss_rate_profiles_selec() + theme(legend.text = element_text(size = 13), legend.key.size = unit(1, "cm"), legend.title = element_text(size = 16))),
        br(),
    ...
    tabPanel(title="combined profiles",
      div(
        br(),
        renderPlot(ss_combined_profiles() * theme(legend.text = element_text(size = 13), legend.key.size = unit(1, "cm"), legend.title = element_text(size = 16)),
                   height = function() {300*ceiling(length(attributes(reac_vals$ss$y)$dimnames[[2]]))}),
        style="overflow-y:scroll; max-height: 950px")
    ),
